...
try:
    ...
    # Fallback if services are not in path or dependencies missing
    print("Warning: Could not import market-intel modules. Ensure services/market-intel is in path.")
    CSVImporter = None
...
class AdPatternMiner:
    ...
    def __init__(self, config_path: Path):
        ...
        self.tracker = CompetitorTracker() if CompetitorTracker else None
        self.ads_data = []
    ...
    def load_from_csv(self, csv_path: str) -> int:
        ...
        """
        if not CSVImporter:
            print("Error: CSVImporter not available.")
            return 0
    ...
    def load_from_tracker(self, days: int = 30, min_engagement: float = 0) -> int:
        ...
        """
        if not self.tracker:
            print("Error: CompetitorTracker not available.")
            return 0
    ...
    def _analyze_hook_patterns(self) -> Dict[str, Any]:
        ...
        for htype, engagements in hook_engagement.items():
            ...
            success_rates[htype] = {
                ...
                "percentage": round(hook_types[htype] / total_ads * 100, 2) if total_ads > 0 else 0,
    ...
    def _analyze_engagement_patterns(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_10_percent": round(sorted_engagements[int(n * 0.9)], 4) if n > 0 else 0,
            "top_25_percent": round(sorted_engagements[int(n * 0.75)], 4) if n > 0 else 0,
    ...
    def _analyze_platform_patterns(self) -> Dict[str, Any]:
        ...
        for platform, count in platform_counts.items():
            ...
            platform_stats[platform] = {
                "count": count,
                "percentage": round(count / len(self.ads_data) * 100, 2) if self.ads_data else 0,
    ...
    def update_config_with_patterns(self, patterns: Dict[str, Any]):
        ...
        if not self.hooks_path.exists():
            ...
            try:
                ...
                print(f"Error updating hooks config: {e}")
