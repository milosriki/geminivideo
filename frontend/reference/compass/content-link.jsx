...
export function ContentLink({ title, description, href, type = 'article', duration }) {
  return (
          ...
          <div>
            ...
            {duration && (
              <>
                ...
                <span className="hidden text-gray-500 sm:inline">{formatDuration(duration)}</span>
              </>
            )}
          </div>
          <p className="text-gray-700 dark:text-gray-400">{description}</p>
          {duration && <div className="text-gray-500 sm:hidden">{formatDuration(duration)}</div>}
        </div>
      </Link>
    </div>
  )
