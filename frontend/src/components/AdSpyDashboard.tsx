...
async function searchAdsLibrary(filters: SearchFilters): Promise<AdLibraryAd[]> {
  let response;
  try {
    response = await fetch(`${API_BASE_URL}/meta/ads-library/search`, {
      method: 'POST',
...
async function getPageAds(pageId: string): Promise<AdLibraryAd[]> {
  let response;
  try {
    response = await fetch(`${API_BASE_URL}/meta/ads-library/page/${pageId}`);
  } catch (error) {
...
async function analyzePatterns(ads: AdLibraryAd[]): Promise<PatternAnalysis> {
  let response;
  try {
    response = await fetch(`${API_BASE_URL}/meta/ads-library/analyze`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
