...
const creativeAPI = {
  ...
  async generateHooks(productInfo: ProductInfo, targetAudience: string, count: number = 5): Promise<GeneratedHook[]> {
    let response;
    try {
      response = await fetch(`${API_BASE_URL}/hooks/generate`, {
        method: 'POST',
  ...
  async analyzeHook(text: string): Promise<{ type: HookType; strength: number; suggestions: string[] }> {
    let response;
    try {
      response = await fetch(`${API_BASE_URL}/hooks/analyze`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async generateAdCopy(
    ...
    let response;
    try {
      response = await fetch(`${API_BASE_URL}/vertex/generate-copy`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          product_name: productInfo.name,
  ...
  async generateImage(
    ...
    let response;
    try {
      response = await fetch(`${API_BASE_URL}/vertex/generate-image`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          prompt,
  ...
  async generateVideo(
    ...
    let response;
    try {
      response = await fetch(`${API_BASE_URL}/vertex/generate-video`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          prompt,
  ...
  async generateStoryboard(
    ...
    let response;
    try {
      response = await fetch(`${API_BASE_URL}/vertex/generate-storyboard`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          product_name: productInfo.name,
  ...
  async generateCTAs(
    ...
    let response;
    try {
      response = await fetch(`${API_BASE_URL}/vertex/generate-cta`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          product_name: productInfo.name,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {generatedHooks.map((hook, idx) => (
            <div
              ...
              className={`p - 4 bg - gray - 800 border rounded - lg cursor - pointer hover: border - indigo - 500 transition - all ${selectedHook?.text === hook.text ? 'border-indigo-500 bg-indigo-900/20' : 'border-gray-700'
                } `}
            >
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(['feed', 'stories', 'reels'] as Platform[]).map((p) => (
                    <button
                      ...
                      className={`px - 3 py - 2 rounded - lg font - semibold text - sm ${platform === p
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className={`px - 3 py - 2 rounded - lg font - semibold text - sm flex items - center justify - center gap - 2 ${deviceType === 'mobile'
                      ? 'bg-indigo-600 text-white'
                      : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
                      } `}
                  >
                  ...
                  <button
                    ...
                    className={`px - 3 py - 2 rounded - lg font - semibold text - sm flex items - center justify - center gap - 2 ${deviceType === 'desktop'
                      ? 'bg-indigo-600 text-white'
                      : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
                      } `}
                  >
