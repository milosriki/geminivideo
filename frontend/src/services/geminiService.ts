...
export const generateVideo = async (prompt: string, image: File | null, aspectRatio: '16:9' | '9:16', onProgress: (message: string) => void) => {
  ...
  if (image) {
    ...
    operation = await (veoAI as any).models.generateVideos({ model: veoModel, prompt, image: { imageBytes: imageBase64, mimeType: image.type }, config });
  } else {
    operation = await (veoAI as any).models.generateVideos({ model: veoModel, prompt, config });
  }
  ...
  while (!operation.done) {
    ...
    operation = await (veoAI as any).operations.getVideosOperation({ operation: operation });
  }
